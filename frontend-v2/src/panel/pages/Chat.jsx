...
export default function Chat() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setMsgs(arr)
      // Notificación si hay mensaje nuevo y no es mío
  ...
  if (noCenter) {
    return (
      <div className="mx-auto max-w-3xl">
        <h1 className="mb-3 text-xl font-bold">Chat interno</h1>
        <div className="card flex flex-col items-center gap-3 p-10 text-center">
          <MessageSquare size={28} className="text-brand-400" />
          <p className="text-dark-200">Elige un centro arriba para abrir su chat.</p>
  ...
  return (
    <div className="mx-auto flex h-[calc(100vh-9rem)] max-w-3xl flex-col">
      <div className="mb-3 flex items-center justify-between">
        <div>
          <h1 className="flex items-center gap-2 text-xl font-bold"><MessageSquare size={20} /> Chat · {center}</h1>
          <p className="text-xs text-dark-500">Solo lo ven los usuarios con acceso a {center}.</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto rounded-lg border border-dark-800 bg-dark-900 p-3">
        {loading ? (
          ...
          <div className="py-12 text-center text-sm text-dark-500">Sin mensajes aún. Sé el primero.</div>
        ) : (
